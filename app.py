...
with st.sidebar:
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        # THE SINGLE BUTTON ACTION
        submitted = st.form_submit_button("🚀 Calculate & Show Report", use_container_width=True)
        
        if submitted:
            if not v_no:
                st.error("Vehicle No required!")
            else:
...
if st.session_state.show_report and st.session_state.vehicles:
    ...
    # Standardize columns for calculation
    numeric_cols = ["Ins_C", "Ins_P", "BB_C", "BB_P", "Fit_C", "Fit_P", "Em_C", "Em_P"]
    for col in numeric_cols:
        if col not in df_raw.columns: df_raw[col] = 0.0

    # Total row calculation
    sums = df_raw[numeric_cols].sum()
    total_row_data = {col: sums[col] for col in numeric_cols}
    total_row_data["Vehicle No."] = "TOTAL"
    total_row_data["Description"] = ""
    
    final_table = pd.concat([df_raw, pd.DataFrame([total_row_data])], ignore_index=True)

    st.write("### 1. Consolidated Abstract")
    st.dataframe(final_table, use_container_width=True)

    # Journal Entries
    total_i_p = sums["Ins_P"]
    total_rm_p = sums["BB_P"] + sums["Fit_P"] + sums["Em_P"]
    
    je_data = [
        ["Dr.", f"{GL_CODES['Prepaid']} Prepaid Expenses", f"{(total_i_p+total_rm_p):,.2f}", ""],
...
st.markdown("---")
st.subheader("🛠️ Manage Current Entries")
if st.session_state.vehicles:
    for v_id in list(st.session_state.vehicles.keys()):
        c1, c2, c3 = st.columns([3, 5, 2])
        c1.write(f"**{v_id}**")
        c2.write(f"{st.session_state.vehicles[v_id]['Description']}")
        if c3.button(f"🗑️ Delete {v_id}", key=f"del_{v_id}"):
            del st.session_state.vehicles[v_id]
            st.rerun()
else:
    st.info("No vehicles added yet. Use the sidebar to enter data and click 'Calculate'.")
